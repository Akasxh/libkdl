...
bars = ax.barh(y_pos, durations_ns, color=kernel_colors, height=0.55, edgecolor='white',
               linewidth=0.8, zorder=3)

# Selection overhead marker: tiny teal vertical line at left edge
for i in range(len(kernels)):
    ax.plot([selection_ns, selection_ns], [i - 0.27, i + 0.27],
            color='#0d9488', linewidth=3.5, zorder=5, solid_capstyle='round')

ax.set_yticks(y_pos)
ax.set_yticklabels(kernels, fontsize=14, fontweight='medium')
ax.invert_yaxis()
...
ax.set_xlabel('Duration (log scale)', fontsize=14, fontweight='medium')
...
ax.text(0.98, 0.02,
        ...
        transform=ax.transAxes, fontsize=13, fontweight='bold', color='#0d9488',
        ha='right', va='bottom',
        bbox=dict(boxstyle='round,pad=0.5', facecolor='#f0fdfa', edgecolor='#0d9488', linewidth=1.2))

# Legend for the teal marker
from matplotlib.lines import Line2D
legend_elements = [Line2D([0], [0], color='#0d9488', linewidth=3.5, label='Selection overhead (5 ns)')]
ax.legend(handles=legend_elements, loc='upper left', fontsize=12, framealpha=0.9)

# Title
ax.set_title('Selection Overhead vs. Real ML Kernels', fontsize=18, fontweight='bold', pad=14, loc='left')
